...
con_string = "mongodb+srv://mohamed_alim:Alifroja1@cluster0.w5jpbmn.mongodb.net/?retryWrites=true&w=majority"
...
def project_api_routes(endpoints):
    @endpoints.route('/get-request', methods=['GET'])
    def hello():
        res = 'Hello world'
        print("Hello world")
        return res

    @endpoints.route('/new-data', methods=['POST'])
    def register_user():
        resp = {}
        try:
    ...
    @endpoints.route('/update-data',methods=['PUT'])
    def update_users():
        resp = {}
        try:
            req_body = request.json
            # req_body = req_body.to_dict()
            user_collection.update_one({"id":req_body['id']}, {"$set": req_body['updated_user']})
    ...
    @endpoints.route('/delete-data',methods=['DELETE'])
    def delete():
        resp = {}
        try:
